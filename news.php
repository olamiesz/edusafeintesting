<h2>EDUSAFE Meeting</h2>
	<p>EDUSAFE Mid Term Review and Technical Meetings will take place at CERN 16 - 20 June 2014.</p>
	<p>
		<a class="btn" 
		href="https://indico.cern.ch/event/307430/">
		More Information <i
			class="icon-circle-arrow-right"></i>
		</a>
	</p>


<h2>EDUSAFE Spring School</h2>
	<p>EDUSAFE Spring School will take place at CERN 3 - 7 March 2014.</p>
	<p>
		<a class="btn" 
		href="https://indico.cern.ch/event/293157/">
		More Information <i
			class="icon-circle-arrow-right"></i>
		</a>
	</p>


<h2>EDUSAFE Kick-Off Week</h2>
	<p>EDUSAFE Kick-Off Week will take place at CERN 28 October - 1 November 2013.</p>
	<p>
		<a class="btn" 
		href="https://indico.cern.ch/conferenceDisplay.py?confId=263532">More Information <i
			class="icon-circle-arrow-right"></i>
		</a>
	</p>
	
<h2>Technical Meeting in Tuscania</h2>
	<p>EDUSAFE Technical meeting in Italy will take place in Tuscania, Italy 5-8 June 2013.</p>
	<p>
		<a class="btn" 
		href="https://indico.cern.ch/conferenceDisplay.py?confId=251541">More Information <i
			class="icon-circle-arrow-right"></i>
		</a>
	</p>

<h2>Kick-Off Meeting</h2>
	<p>EDUSAFE's kick-off meeting will be organized at CERN on Monday 17th
		of September, 2012.The goal of the meeting
		is to outline the scientific scope of the project and give a short
		overview of the administrative procedures.</p>
	<p>The 1st Supervisory Board will be held as part of the kick-off
		meeting.</p>
	<p>
		<a class="btn"
			href="https://indico.cern.ch/conferenceDisplay.py?confId=202302">More
			Information <i class="icon-circle-arrow-right"></i>
		</a>
	</p>

	<!--<h2>Job Openings !</h2>
	<p>EDUSAFE is currently recruiting 2 Experienced researchers.</p>
	<p>
		<a class="btn" href="?page=jobs">More Information <i
			class="icon-circle-arrow-right"></i>
		</a>
	</p>-->
	<h2>Web Site Launch</h2>
	<p>The EDUSAFE web site is launched.</p>
	<p>
		<a class="btn" href="mailto:atlas-edusafe-webmaster@cern.ch">Send
			Comments &amp; Feedback <i class="icon-envelope"></i>
		</a>
	</p>

